...
contract LostStaking is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    ...
    modifier isExpired() {
        require(0 < ((expiration - block.timestamp) / 60 ), "Close Staking");
        _;
    }
    ...
    function setOneDayPrev(address account, uint256 tokenId, uint256 _days) public onlyOwner(){
        _depositBlocks[account][tokenId] = _depositBlocks[account][tokenId] - ( _days * 1 days);
    }
    ...
    function setExpiration(uint256 _expiration) external onlyOwner() {
        expiration = expiration + (_expiration * 1 days);
    }
    ...
    function deposit(markelData[] memory _markelData) public {
        ...
        for (uint256 i; i < _markelData.length; i++) {
            ...
            _depositBlocks[msg.sender][_markelData[i]._indexNum] = blockCur1;
