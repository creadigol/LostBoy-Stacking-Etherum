...
const Stake = (props:any) =>{
        ...
        if(unstakingNft.length > 0){
            ...
            .on('transactionHash', function(hash:any){
                waitForReceipt(hash, function (response:any) {
                    if(response.status){ 
                        alert("Lostboys were unstaked successfully");
                        setTimeout(() => {
                            props.pageRefresh();
                            // getBalanceOfnft();
                        }, 500);
    ...
    const handleSelectAll = (e:any) => {
        ...
        setUnstakingNft(nfts.map(li => li.id));
        if (isCheckAll) {
            setUnstakingNft([]);
        }
    };
                                    ...
                                    {nfts.map((d: any, i:any) => (
                                        <Col key={i} xl={4} md={4} sm={4} xs={4} >
                                            <div className="unstake_list" >
                                                <input type="checkbox" id={d.id} value={d.id} onChange={(e) => onIngredientChange(e)} checked={unstakingNft.includes(d.id)}/>
                                                <label htmlFor={d.id} >
